...
try:
    ...
    Ada_LEval_enable = True
except:
    print("Ada_LEval is not enabled.")
    Ada_LEval_enable = False
...
def sample_stackselect_requests(
    dataset_path: str,
    num_requests: int,
...
"""
    ...
    lt = max(len(dataset), num_requests)
    ...
    for i in range(lt):
        ...
        completion = build_answer(dataset, i)
...
def sample_textsort_requests(
    ...
    def build_answer(dataset, line):
        if isinstance(line, int):
            line = dataset[line]
        assert isinstance(line, dict)
        return [ str(id) for id in line['answer']]
    # Load the dataset.
    with open(dataset_path, encoding='utf-8') as f:
        dataset = json.load(f)
...
def sample_sharegpt_chat_requests(
    dataset_path: str,
    num_requests: int,
    tokenizer: PreTrainedTokenizerBase,
    fixed_output_len: Optional[int] = None,
...
def main(args: argparse.Namespace):
    ...
    elif args.dataset_name == "stackselect":
        assert Ada_LEval_enable, "Ada_LEval is not enabled, while stackselect dataset is selected."
        input_requests = sample_stackselect_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            fixed_output_len=args.sharegpt_output_len,
    ...
    elif args.dataset_name == "textsort":
        assert Ada_LEval_enable, "Ada_LEval is not enabled, while textsort dataset is selected."
        input_requests = sample_textsort_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            fixed_output_len=args.sharegpt_output_len,
